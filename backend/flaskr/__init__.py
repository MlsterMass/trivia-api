...
def create_app(test_config=None):
    ...
    def create_question():
        ...
        if (len(new_question) == 0) or (len(insert_answer) == 0) or (len(insert_answer) == 0) or (
                len(insert_answer) == 0):
            abort(422)
    ...
    def search_questions():
        ...
        related_questions = Question.query.filter(Question.question.ilike(f'%{search_term}%')).all()
    ...
    def not_processable(error):
        return jsonify({
            'success': False,
            'error': 422,
            'message': 'not processable. Syntax error.'
        }), 422
    ...
    def internal_server(error):
        return jsonify({
            'success': False,
            'error': 500,
            'message': 'Sorry, the faut is us not you. Please try again later.'
        }), 500
